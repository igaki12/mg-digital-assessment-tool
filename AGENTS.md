...
3.  **Measurement Views**
    ...
    *   音声ガイドを設けるページでは、ページ表示時に導入音声を自動再生し、`もう一度聞く` ボタンと音量調整 UI を同じカード内に配置してよい
...
13. **検査ページの導入音声について:**
    *   眼・腕・歩行・姿勢・表情・音声の各検査ページでは、ページを開いた時点で検査名と概要を伝える導入音声を設けてよい。
    *   導入音声を設けるページでは、音声ガイドの音量を調整できる UI を併設してよい。
    *   導入音声の wav ファイルはルート直下に置かず、`public/audio` 配下で管理する。
