# AGENTS.md - DigiNeuro Marker (Project Draft)
...
- 認証、同意、クラウド保存はバックエンド前提の別フェーズだが、現行UIではダミーログイン、利用規約同意、ダミークラウド送信導線を実装済みとして扱う
- 測定データは、個人情報保護を徹底した上で診療・研究利用される前提の文言に統一する。旧来の「すべて端末内で完結」表現は使わない
- 要件書を更新するときは、現行実装 / 次期機能 / 研究課題を混在させない

## 1. プロジェクト概要
**アプリ名称:** 日本語名 `デジニューロマーカー`、英語名 `DigiNeuro Marker`（Digital Neuro Biomarker からの造語）。

**目的:** パーキンソン病・重症筋無力症（MG）などの神経疾患患者が、自宅で症状変化を記録・管理できるWebアプリケーションの開発。現行実装はMG中心のプロトタイプとして扱う。
**特徴:**
...
*   **プライバシーと診療・研究利用:** MediaPipe解析はブラウザ上で実行する。測定データは個人情報保護を徹底した上で診療・研究利用される想定とし、実運用では安全なサーバー送信・保管を前提にする。
*   **認証と同意:** アプリ全体をログイン必須にし、未ログイン時は `/login` に遷移する。現行は任意入力でログインできるダミー認証で、利用規約同意が必要。
*   **医師連携:** `医師共有` 画面からクラウド送信を模したダミーUIで未同期件数・送信導線・動画整理導線を確認できるようにする。
...
*   **Storage:** IndexedDB (動画・音声・時系列データ用), LocalStorage / SessionStorage (設定・軽量データ・ダミー認証状態用)
...
*   **タスク:** 上方視試験 (Upward Gaze Test)
    *   ユーザーはカメラに向かい、頭を動かさずに視線だけを可能な限り上に向ける動作を60秒間維持する。
*   **計測ロジック:**
...
*   **タスク:** 上肢挙上持続試験 (Arm Abduction Time)
    *   両腕を肩の高さ（90度）まで水平に広げ、その状態を90秒間維持する。
*   **計測ロジック:**
    *   肩・肘・手首のランドマーク角度を計算。
    *   左腕の肩角度は左右反転による符号差を補正するため、算出角度を `180 - X` として右腕と同じ向きの値に揃える。
    *   腕の角度が90度を下回った時点、または震え（振幅）が大きくなった時点を「限界」として記録。
*   **UI/UX:**
    *   画面上にガイドライン（ボックス）を表示し、「この枠内に腕を維持してください」と指示。
    *   計測中は30秒、60秒、残り10秒、90秒終了時の音声ガイドを再生する。
...
#### B-2. 3m立ち上がり歩行テスト (3m Stand-up Walk)
*   **使用モデル:** MediaPipe Pose Landmarker
*   **動作フロー:**
    1.  開始前に「3mを測って床に線を引く」「椅子を用意して座る」「障害物をどける」「必要に応じて家族・介助者が見守る」を表示する。
    2.  カメラ起動後、全身が入ったら `計測開始` を有効化する。
    3.  患者が椅子から立ち上がり、3m先で方向転換して戻り、座ったらユーザーが `停止して保存` を押す。
*   **計測データ:**
    *   合計時間、立ち上がり推定時間、方向転換推定時間、復路・着座推定時間。
    *   歩数、平均/最大歩行速度、左右膝角度。
    *   v1は半自動方式で、開始・停止はユーザー操作。区間推定や歩数推定には `inferenceNotes` で注意を残す。
...
現行プロトタイプではブラウザ内の IndexedDB / Storage に保存する。実運用では個人情報保護を徹底した上で、診療・研究利用を目的としたサーバー送信・保管を前提にする。

**Database:** `IndexedDB` (名称: `MG_Assessment_DB`)

*   **Store: `sessions`** (評価セッションごとのメタデータ)
    *   `id`: Timestamp
    *   `type`: "ptosis" | "limbs" | "gait" | "tug" | "posture" | "expression" | "voice" | "epro"
    *   `date`: ISO String
    *   `summary_score`: (例: EAR平均値, 歩行速度平均, 姿勢スコア, 問診票合計点)
*   **Store: `time_series_data`** (グラフ描画用)
    *   `session_id`: 参照キー
    *   `frame_data`: Array of objects `{ timestamp, earLeft, earRight, armLeftDeg, armRightDeg, gaitSpeed, kneeLeftDeg, kneeRightDeg, tugElapsedSec, tugStepCount, tugPhase, trunkFlexionDeg, blinkRatePerMin, voicePitchHz, ... }`
    *   `details`: 検査サマリーや問診票内訳などの補足オブジェクト
    *   TUG / 3m立ち上がり歩行テストの `details`: `totalDurationSec`, `standUpSec`, `walkOutSec`, `turnSec`, `returnAndSitSec`, `stepCount`, `avgGaitSpeed`, `maxGaitSpeed`, `avgKneeLeftDeg`, `avgKneeRightDeg`, `inferenceNotes`
...
1.  **Login**
    *   アプリ全体をダミーログイン必須にし、未ログイン時は `/login` にリダイレクトする。
    *   任意のログインID/メールアドレスとパスワードでログインできる。
    *   利用規約モーダル、利用規約ダウンロード、同意チェック、ログイン状態保持チェックを持つ。
    *   `ログイン状態を保持する` がONなら `localStorage`、OFFなら `sessionStorage` にダミー認証状態を保存する。
    *   タイトルは `DigiNeuro Marker` を主表示とし、日本語名 `デジニューロマーカー` は説明文中で扱う。
2.  **Home / Dashboard**
    *   「検査を始める」（新規測定）
    *   「記録を見る」（カレンダー・グラフ表示）
    *   「設定」（身長設定、データ削除）
    *   ヘッダーロゴと hero の小見出しは `DigiNeuro Marker` に統一し、旧 `MG Digital Assessment` 表記は使わない。
    *   モバイルのハンバーガーメニューは、開閉時に縦幅・透明度・位置が滑らかに変化するアニメーションを付ける。
...
3.  **Select Assessment (Menu)**
    *   眼の検査 (Ptosis)
    *   腕の検査 (Upper Limb)
    *   歩行監視モード (Gait Monitor)
    *   3m立ち上がり歩行テスト (3m Stand-up Walk)
    *   姿勢の検査 (Posture)
    *   表情の検査 (Expression)
    *   音声の検査 (Voice)
    *   問診票 (Questionnaire)
    *   検査カードの英語名は subtitle / badge 的な小さい表示にする。
4.  **Records / 記録を見る**
    ...
    *   詳細では生の timestamp 一覧を主表示にせず、検査ごとの時系列グラフを優先表示する
    *   例として Ptosis では、有効時間内の `時間 - 左右 EAR` を 2 本線グラフで確認できるようにする
...
5.  **Measurement Views**
    ...
    *   カメラ検査では `CameraOverlay` 右下に前面/背面カメラ切替ボタンを表示する。
    *   前面/背面カメラ切替は `src/utils/camera.ts` の `openCameraStream()` と `getNextCameraFacingMode()` を使い、`facingMode: { ideal: ... }` を指定する。
    *   計測中・録画中のカメラ切替はデータ破損を避けるため無効化し、開始前または位置合わせ中に切り替え可能にする。
...
6.  **Doctor's Share View / 医師共有**
    *   未同期件数・未同期データ量・未同期期間・最終送信結果を確認できるサマリーを表示
    *   アプリ全体ログイン必須化により、ページ内ログインUIや利用規約同意チェックは表示しない
    *   `クラウドへ送信する` 実行後のみ、段階的な送信進捗パネルを表示する
    *   同期対象一覧を表示し、その末尾に `同期済み動画を削除して容量を確保` ボタンと確認UIを配置する
7.  **Scroll Behavior**
    *   Dashboard 以外のページを開いた際は、`Layout` 側で最上部へ smooth scroll し、途中位置から表示されないようにする。
...
*   **眼瞼下垂テスト:**
    *   *Bad:* 「上方視位を保持してください」
    *   *Good:* **「頭を動かさずに、目だけで天井を見てください。そのまま60秒キープします」**
*   **歩行監視モード:**
    *   *Bad:* 「監視待機中」
...
*   **ログイン・利用規約:**
    *   `DigiNeuro Marker` を主タイトルとして扱い、深緑系グラデーションや研究的なエフェクトを使ってよい。
    *   利用規約はドラフトであり、正式運用前に所属機関、倫理審査委員会、法務担当者または弁護士の確認が必要である旨を明記する。
    *   免責は「故意または重過失がある場合を除き」といった例外を含め、「一切責任を負いません」のような全部免責表現を避ける。
*   **フィードバック:**
...
3.  **医師へのデータ共有方法:**
    *   現行プロトタイプでは端末内データをもとに、医師共有画面でサーバー送信を模したダミーフローを確認できるようにする。
    *   実運用では個人情報保護を徹底したサーバー送信・保管を前提にする。
    *   **追加:** 可能であればCSV出力を提供する。
4.  **測定画面の操作導線について:**
...
8.  **医師共有（ダミークラウド送信）画面について:**
    ...
    *   ログイン状態と利用規約同意はアプリ全体のログイン画面で扱う。`Review` ページ内では送信確認とダミー進捗に集中する。
    *   同期済み状態、最終送信結果はページ内 state のみで保持し、再読込では引き継がない。
    *   送信進捗パネルは送信開始前は表示しない。
...
9.  **Ptosis 再計測時の MediaPipe ループ管理について:**
    *   60秒計測完了後に `もう一度測定` を押した際、古い `requestAnimationFrame` ループや停止済み `video` を再利用しない。
...
10. **Ptosis 画面のフェーズ表示とオーバーレイについて:**
    ...
    *   `CameraOverlay` はカメラ検査共通の上部ガイド、中央表示、右下カメラ切替ボタンを担う。個別ページで独自の overlay DOM を増やす前に共通化を検討する。
    *   右下の前面/背面カメラ切替ボタンは `CameraOverlay` props (`cameraFacingMode`, `onSwitchCamera`, `isCameraSwitching`, `isCameraSwitchDisabled`) で制御する。
...
13. **検査ページの導入音声について:**
    ...
    *   ルート直下に新規 wav が置かれた場合は、既存の命名規則に合わせて `public/audio` へ移動し、必要に応じて `docs/audio` はビルドで同期する。
    *   `Audio_Phrases_by_Inspection_Item.md` には対応する具体的な wav ファイル名を記載し、未作成の音声は「未作成」と英語のファイル名案を明記する。実装済みになったものは未作成表記を残さない。
...
14. **Records / 記録を見る ページの構成について:**
    ...
    *   詳細データは `pre` 形式の表よりも検査別グラフを優先し、必要な場合のみテキスト表示をフォールバックにする。
    *   Ptosis は `時間 - 左右 EAR`、Limb は `時間 - 左右腕角度`、Voice は `タスク別の平均音量/平均ピッチ` のように、検査内容に自然な系列を出し分ける。
    *   3m立ち上がり歩行テストでは、代表値の初期表示を `合計時間` とし、`合計時間`, `立ち上がり時間`, `歩数`, `平均歩行速度`, `左膝角度`, `右膝角度` を選択可能にする。
    *   3m立ち上がり歩行テストの詳細では、速度・膝角度・歩数推移を表示し、推定不能な指標は空データまたは `inferenceNotes` として扱って既存レコード表示を壊さない。
    *   モバイル詳細モーダルは閉じるアイコンボタンを使い、表示中は背景スクロールをロックする。
15. **3m立ち上がり歩行テスト（TUG由来）について:**
    *   画面表示名は `3m立ち上がり歩行テスト`、英語 subtitle は `3m Stand-up Walk` とし、ユーザー向けには `TUG` 単独表記を避ける。
    *   既存の歩行監視モードとは別検査として扱い、`AssessmentType` は `"tug"` を使う。
    *   v1では開始・停止をユーザー操作とし、完全自動の座位・立位・着座判定は研究課題扱いにする。
    *   3m距離の厳密な実測はアプリ側で保証せず、準備ガイドでユーザーに明示する。
16. **表情検査について:**
    *   `普通10秒 → 笑顔5秒` を4セット繰り返すフローとする。
    *   自然表情と笑顔の各フェーズで、顔が入っていることを確認してから保持計測へ進める。
17. **姿勢検査について:**
    *   正面と側面をそれぞれ5秒ずつ検査する。求めるポーズを検出しても即座に次へ進めず、5秒間の保持計測を完了する。
    *   求めるポーズを検出したら、正面・側面それぞれ1枚ずつ写真を保存する。
